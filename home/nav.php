<body>
<nav class="navbar sticky-top navbar-expand-lg bg-dark-custom_home">
    <div class="row width_hundred_home">
      <div class="col col-lg-8">
      <a class="navbar-brand logo_home" href="#">Logo</a>
      <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
        <i class="fas fa-bars"></i>
      </button>
      </div>
      <div class="collapse navbar-collapse col col-lg-4" id="navbarSupportedContent">
        <ul class="navbar-nav mr-auto w-100 justify-content-end">
          <li class="nav-item">
            <a class="nav-link link_formatter text_white" href="#">ANOUNCEMENT</a>
          </li>
          <li class="nav-item">
            <a class="nav-link link_formatter text_white" href="#">ABOUT US</a>
          </li>
          <li class="nav-item">
            <a class="nav-link link_formatter text_white" href="#">LOGIN</a>
          </li>
        </ul>
      </div>
    </div>
  </nav>
